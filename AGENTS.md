# AGENTS.md — Astrolabio

Instrucciones para cualquier agente de código que trabaje en este
repositorio. Claude Code las carga a través de `CLAUDE.md`, que solo importa
este archivo. Léelo antes de escribir código, proponer arquitectura o crear
archivos.
...
la regla §2.3 de AGENTS.md, que existe por X; ¿la derogamos?». Las reglas de
...
1. Este archivo entero. Las invariantes del §2 no se rompen sin derogar su
   ADR.
2. [`docs/PRD.md`](docs/PRD.md): qué se construye, para quién y qué no entra.
3. [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md): cómo se conectan las
   piezas y dónde vive cada cosa.
4. [`docs/DESIGN_SYSTEM.md`](docs/DESIGN_SYSTEM.md), si el cambio toca la
   interfaz.
5. El alcance vigente (§7) y los ADR que toque el cambio.

Antes de crear un componente, una función o un patrón, busca si ya existe
(§4).
...
El producto completo —flujos, requisitos, métricas y lo que no entra— está en
[`docs/PRD.md`](docs/PRD.md). El análisis original, con las alternativas
descartadas, es la nota `Propuesta-Astrolabio` del vault personal de Johan (no
está en este repo).
...
Cómo encajan estas piezas —servicios, montajes y el camino de una petición—
...
- **Interfaz**: sigue [`docs/DESIGN_SYSTEM.md`](docs/DESIGN_SYSTEM.md). Un
  panel, botón, campo o aviso nuevo usa las clases de los que ya existen; un
  patrón nuevo se documenta allí antes de repetirse.
...
## 7. Documentos y registros de decisión

- Producto: [`docs/PRD.md`](docs/PRD.md)
